...
#while a for loop is for iterating over a sentence the difference is that the while loop will keep repeating 
#til its stopped while the for loop is be used one singular time 

# 2. Name and describe three python operator families. Plese write your response using complete
# sentences.
#ans: arithmetic is basically just numbers and math assignment varibles are used to assign values to variables
#and the comparison is to compare the value of 2 values
...
#ans:
def multiply_and_subtract(8, 9, 10):
    print(result)
...
# argument. 
#ans:the range function is made to put down a list of numbers so to make a function that takes 2 arguments
if 1 in range(1, 21):
    multiply 1 with 21
    print(result)
